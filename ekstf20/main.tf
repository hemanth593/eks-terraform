...
%{if lookup(each.value.tags, "Type", "") == "argocd"}"workload" = "argocd"%{endif}

%{if lookup(each.value.tags, "Type", "") == "argocd"}[settings.kubernetes.node-taints]
"argocd=true:NoSchedule"%{endif}
EOF
  )
}
